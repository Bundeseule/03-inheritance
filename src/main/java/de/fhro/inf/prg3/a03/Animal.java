...
public class Animal {
    ...
    public Animal(GenusSpecies genusSpecies, String name, GenusSpecies[] devours, int sleep, int awake, int digest, int collectionAmount) {
        ...
        if (awake < digest)
            throw new IllegalArgumentException("Makes no sense that an animal needs more time for digesting that it is awake!");
        this.name = name;
    ...
    public void feed() {
        if (!isHungry()) throw new IllegalStateException("Can't stuff a cat...");
